...
###  
def worker_init(G, clusters, tree, lookup_table_filename, layer, UB):
    global mp_G
    global mp_clusters
    global mp_tree
    ...
    global mp_succs
    global mp_layer
    global mp_clustered_lookup_table
    ...
    #####  to clusterize the lookup table ################
    mp_clustered_lookup_table = {}
    for v in clusters[start_cluster_id]:
        for sigma in layer:
            s_sigma = set(sigma)
            l_sigma = len(sigma)
            mp_clustered_lookup_table[(tuple(sigma), v)] = [state for state in mp_lookup_table.values() 
            if set(state.sigma).issubset(sigma) and len(state.sigma) = l_sigma - 1 and state.v == v]
    #####

    print([(state.sigma, state.v) for state in mp_lookup_table.values()])
    print(mp_clustered_lookup_table)
...
def parallel(sigma_id):
    ...
    for ind_V_j in [ind for ind in sigma if can_be_the_last_cluster(sigma, ind, mp_transitive_closure)]:
        ...
        for v in mp_clusters[start_cluster_id]:
            ...
            key = (tuple([start_cluster_id] + truncated_sigma), v)
            if key in mp_clustered_lookup_table:
                filtered_prev_states = mp_clustered_lookup_table[key]
                #==================================
                ...
                cutoff += len(mp_clusters[ind_V_j])
...
def compute_Bellman_layer(G, clusters,  layer_level, previous_layer, tree, lookup_table_name, keep_lookup_table, workers_count, predicted_workers_count, UB, LB):
    ...
    if layer_level < 1: # baseline case
        ...
        with mp.Pool(actual_workers_count, worker_init, (G, clusters, tree, f'{lookup_table_name}{layer_level-1:03d}.dct', layer, UB), maxtasksperchild=MAXTASKSPERWORKER) as pool:
            ...
            results = pool.map(parallel, range(len(layer)))
            #==================================
